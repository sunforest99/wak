...
public class NpcYung : Npcdata
{
    ...
    public override void checkQuest()
    {
        ...
        if (!GameMng.I.userData.quest_done.Contains( (int)QUEST_CODE.FISH ))
        {
            // 퀘스트를 완료한건 아니지만 퀘스트 수령은 한 상태
            if (Character.sub_quest.ContainsKey(QUEST_CODE.FISH.ToString()))
            {
                // 진행도 0  =>  퀘스트 수령만 하고 진행 하나도 안함
                if (Character.sub_quest_progress[QUEST_CODE.FISH.ToString()].Equals(0))
                {
                    base.dialogs = Quest_Fishing_Check();
    ...
    protected IEnumerator Quest_Fishing()
    {
        yield return "무의식 낚시";
        ...
        setQuestIcon();
        GameMng.I.StartSubQuest(QUEST_CODE.FISH);
        
    }

    protected IEnumerator Quest_Fishing_Check()
    {
        bool check = false;
        try {
            int idx = Character.haveItem[2].FindIndex(name => name.itemData.itemIndex == ITEM_INDEX.BOKUH);
        ...
        if (check) {
            ...
            GameMng.I.nextSubQuest(QUEST_CODE.FISH);
        } else {
            yield return "...";
