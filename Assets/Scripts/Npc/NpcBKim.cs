...
{    
    ...
    public override void checkQuest()
    {
        ...
        if (!GameMng.I.userData.quest_done.Contains( (int)QUEST_CODE.GOHON ))
        {
            // 퀘스트를 완료한건 아니지만 퀘스트 수령은 한 상태
            if (Character.sub_quest.ContainsKey(QUEST_CODE.GOHON.ToString()))
            {
                // 진행도 0  =>  퀘스트 수령만 하고 진행 하나도 안함
                if (Character.sub_quest_progress[QUEST_CODE.GOHON.ToString()].Equals(0))
                {
                    base.dialogs = Quest_Gohon_Check();
    ...
    protected IEnumerator Quest_Gohon()
    {
        ...
        setQuestIcon();
        GameMng.I.StartSubQuest(QUEST_CODE.GOHON);
        
    }

    protected IEnumerator Quest_Gohon_Check()
    {
        bool check = false;
        ITEM_INDEX[] girlsFv = {
            ITEM_INDEX.KANOLAYU,
            ITEM_INDEX.AIR,
            ITEM_INDEX.HAIR_ROLE
        };

        try {
            for (int i = 0; i < girlsFv.Length; i++) {
                int idx = Character.haveItem[1].FindIndex(name => name.itemData.itemIndex == girlsFv[i]);

                // 아이템 개수가 안맞으면 이 함수에 들어올수 없지만 혹시 모르니
                if (idx >= 0)
                {
                    if (Character.haveItem[1][idx].itemCount > 1) {
                        Character.haveItem[1][idx].itemCount--;
                        // GameMng.I.userData.inventory[2][idx].mount--;
                    } else if (Character.haveItem[1][idx].itemCount == 1) {
        ...
        if (check) {
            ...
            GameMng.I.nextSubQuest(QUEST_CODE.GOHON);
        } else {
            yield return "...";
